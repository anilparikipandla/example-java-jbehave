...
public class ApiSteps {
    ...
    @Given("Precondition")
    @Composite(steps = {"Given preparedStep1",
            "Given preparedStep2"})
    ...
    public void getDashboards(final ExamplesTable table) {
        logger.info("Get Dashboards with: projectName {}, dashboardId {}", table.getRow(0).get("projectName"), table.getRow(0).get("dashboardId"));
    }

    @When("Get Dashboard: $table")
    @Alias("Request Dashboard: $table")
    public void getDashboard(final ExamplesTable table) {
        logger.info("Get Dashboard with: projectName {}, dashboardId {}", table.getRow(0).get("projectName"), table.getRow(0).get("dashboardId"));
    }
